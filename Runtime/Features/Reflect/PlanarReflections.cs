...
namespace LYU.WaterSystem
{
    ...
    public class PlanarReflections : MonoBehaviour
    {
        ...
        {
            public ResolutionMulltiplier m_ResolutionMultiplier = ResolutionMulltiplier.Quarter;
            public float m_ClipPlaneOffset = 0.07f;
            public LayerMask m_ReflectLayers = -1;
            public bool m_ReflectShadows;
            public Camera targetCamera;

            public PlanarReflectionSettings()
            {
                m_ReflectLayers &= ~(1 << Water.WaterLayer);
            }
        }
        ...
        [HideInInspector] public float m_planeOffset;
        ...
        private static Dictionary<Camera, WaterData> _reflectTargetCache = new Dictionary<Camera, WaterData>();
        ...
        private void UpdateCamera(Camera src, Camera dest)
        {
            ...
            if (_cameraData == null)
                _cameraData = dest.GetComponent<UniversalAdditionalCameraData>();
            _cameraData.renderShadows = m_settings.m_ReflectShadows; // turn off shadows for the reflection camera
        }
        ...
        private void UpdateReflectionCamera(Camera realCamera)
        {
            ...
            if (plane != null)
            {
                pos = plane.transform.position + Vector3.up * m_planeOffset;
        ...
        public static float GetScaleValue(ResolutionMulltiplier resolution)
        {
            switch (resolution)
            {
        ...
        private Camera CreateMirrorObjects()
        {
            ...
            _cameraData = go.AddComponent(typeof(UniversalAdditionalCameraData)) as UniversalAdditionalCameraData;
        ...
        private void PlanarReflectionTexture(Camera cam)
        {
            ...
            if (_reflectionTexture == null || _reflectionTexture.width != res.x || _reflectionTexture.height != res.y)
            {
                if (_reflectionTexture)
                    RenderTexture.ReleaseTemporary(_reflectionTexture);
                ...
                waterData.reflectRt = _reflectionTexture;
            }
        ...
        {
            var x = (int) (cam.pixelWidth * scale * GetScaleValue(m_settings.m_ResolutionMultiplier));
            var y = (int) (cam.pixelHeight * scale * GetScaleValue(m_settings.m_ResolutionMultiplier));
            return new int2(x, y);
        }

        //todo : 反射相机光照只算diffuse+GI
        //todo: Unity还提供了Per Layer Culling Distance，即对不同层可以使用不同的有效渲染距离，对小物体所在的层，可以进行一个动态的剔除
        private void ExecutePlanarReflections(ScriptableRenderContext context, Camera camera)
        {
            ...
            switch (camera.cameraType)
            {
                case CameraType.Game:
                    if (m_settings.targetCamera == null)
                    {
                    ...
                    {
                        if (m_settings.targetCamera != camera)
                            return;
                    }
            ...
            if (_reflectTargetCache.TryGetValue(camera, out var cacheData))
            {
                if (waterData.material != null)
                    waterData.material.SetTexture(_planarReflectionTextureId, cacheData.reflectRt);
                return;
            }

            _reflectTargetCache.Add(camera, waterData);
            ...
            if (waterData.material != null)
                waterData.material.SetTexture(_planarReflectionTextureId, _reflectionTexture);
        }
